...
export const creatNewSessions = (req, res) => {
  const { title, date, nots } = req.body;
  const { id:lawsuites_id } = req.params;
console.log(lawsuites_id,title, date, nots )
  if (!title || !date || !nots || !lawsuites_id) {
    return res.status(400).json({ message: 'Missing required fields' });
  }

  const query = `
    INSERT INTO sessions (title, date, nots, lawsuites_id)
    VALUES (?, ?, ?, ?)
  `;

  const params = [title, date, nots, lawsuites_id];
